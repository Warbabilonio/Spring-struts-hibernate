...
<html:form action="/guardar" focus="persona.nombre">
	...
	<table>
		...
		<tr>
			<th><bean:message key="persona.fechalta" />:</th>

			<td><c:choose>
					<c:when test="${not empty param.idPersona}">
						 <html:text property="persona.fechalta" />
      				</c:when>
					<c:otherwise>
						<html:text property="persona.fechalta" readonly="true"/>
     				</c:otherwise>
				</c:choose> <%-- 				 <html:text property="persona.fechalta" /> --%></td>
		</tr>
		<tr>
